import { getCompiledFile, runWithClientCompileContext } from "../client/mod.ts";
...
async function executeMatched(
  ...
  try {
    ...
    return await recover(thrown, matched.boundary, ctx, isPartial, fatal);
  }
}
    ...
    try {
      ...
      if (!matched) {
        ...
        return await runPipeline(
          ctx,
          miss.middleware,
          () => executeNotFound(ctx, miss.boundary, isPartial, fatal),
        );
      }
      const ctx: RequestCtx = {
        req,
      ...
      return await runPipeline(
        ctx,
        matched.middleware,
        () => executeMatched(ctx, matched, isPartial, fatal),
      );
    } catch (thrown) {
