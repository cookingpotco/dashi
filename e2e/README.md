...
`withBrowser` and stop. If it needs a new path, add a feature folder that
...
`main.ts` boots it, and every page path is a prefixed feature group, matching
the examples. Put a new folder next to it only when the behaviour cannot live on
