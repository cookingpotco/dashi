...
- **Name the test layer.** Unit tests for pure functions (JSX runtime
  exports, calculations). Black-box tests for behaviour, at the outer layer
  that runs: compiled JSX over a twin `jsx()` call, HTTP once COO-38 lands.
  One test per flow; cover as much of that flow as will hold. Unrelated
  flows stay separate. HTTP coverage waits on COO-38 — plan a
  `TODO(COO-38)`, not a new harness.
