...
- Client JS attaches only via `client.module` / `client.element` at module
  ...
  `/_dashi/*` is reserved.
- Element cache policy is `cached()` + `CacheConfig`. Omitted `cached()` on an
  Element response is `private`. That type is also `staticFile`’s cache argument
  (omitted `staticFile` cache stays immutable). Raw `Cache-Control` / `Vary`
  stay on `Response` and middleware. The most specific `cached()` wins (handler
  over inner layout over root layout).
