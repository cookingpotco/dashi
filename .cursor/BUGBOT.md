...
- Root `deno.json` `imports` may map a bare specifier to a dependency, never to
  ...
  `compilerOptions.jsxImportSource` on the root config. `compilerOptions.lib` is
  `["dom", "deno.ns", "deno.unstable"]` on this package and on every app
...
- Client JS attaches only via `client.module` / `client.element` at module
  ...
  tree has a form; other module scripts are added only when that host rendered.
  A route slot `import()`s its `Link` modulepreloads before swap. `staticFile`
  is app-mounted disk files. Flag a second include, bundle, or inject path.
